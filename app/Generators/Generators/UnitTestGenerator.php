...
class UnitTestGenerator extends BaseGenerator
{
    public function generate(): bool
    {
        ...
        $deleteAssertion = $this->commandData->withSoftDeletes
            ? "\$this->assertSoftDeleted('{$tableName}', ['id' => \${$modelVar}->id]);"
            : "\$this->assertDatabaseMissing('{$tableName}', ['id' => \${$modelVar}->id]);";
        ...
        switch ($dbType) {
            ...
            case 'mediumInteger':
                if ($field->belongsTo) {
                    ...
                    return "\\App\\Models\\{$relatedModel}::factory()->create()->id";
                } elseif (str_ends_with($fieldName, '_id') && $fieldName !== 'id') {
                    $relatedModel = \Illuminate\Support\Str::studly(substr($fieldName, 0, -3));
                    return "(class_exists(\\App\\Models\\{$relatedModel}::class) ? \\App\\Models\\{$relatedModel}::factory()->create()->id : 1)";
                } elseif ($fieldName === 'stock' || str_contains($fieldName, 'quantity') || str_contains($fieldName, 'qty')) {
