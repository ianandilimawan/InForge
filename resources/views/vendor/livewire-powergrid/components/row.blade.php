...
@foreach ($columns as $column)
    @php
        ...
        if ($content instanceof \UnitEnum) {
            $content = $content instanceof \BackedEnum ? $content->value : $content->name;
        }
                    ...
                    <div wire:replace.self>
                        @if (data_get($column, 'isAction'))
                            <div x-data="pgRenderActions({ rowId: @js(data_get($row, $this->realPrimaryKey)), parentId: @js($parentId) })"
                                class="{{ theme_style($theme, 'table.body.tdActionsContainer') }}" x-html="toHtml">
                            </div>
                        @endif
                    </div>
                </div>
            ...
            @else
                <span @class([$contentClassField, $contentClass])>
                    @if (filled($templateContent))
                        <div x-data="pgRenderRowTemplate({
