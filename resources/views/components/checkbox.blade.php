...
@php
    $id = $id ?? ($name . ($value !== '1' ? '_' . \Illuminate\Support\Str::slug($value) : ''));
    ...
    <label for="{{ $id }}"
        ...
        <div class="flex items-center shrink-0 {{ $description ? 'mt-0.5' : '' }}">
            <input type="checkbox"
                ...
                {{ $isChecked ? 'checked' : '' }}
                {{ $disabled ? 'disabled' : '' }}
                {{ $attributes->merge(['class' => "w-4 h-4 rounded-md border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 {$accentClass} focus:ring-2 focus:ring-emerald-500/30 focus:ring-offset-0 focus:outline-none transition cursor-pointer disabled:cursor-not-allowed"]) }}>
        </div>
