...
@section('content')
    <div class="space-y-6">
        ...
        <div>
            ...
            <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">Update permission information and slug</p>
        </div>
...
@if ($errors->any())
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700">
            <form x-data="ajaxForm" @submit.prevent="submit" action="{{ route('admin.permissions.update', $permission) }}" method="POST"
                class="lg:p-8 px-4 py-4 space-y-6">
                @csrf
                @method('PUT')

                <!-- Name -->
                <div class="mb-6">
                    <x-input-floating type="text" name="name" label="Name" value="{{ old('name', $permission->name) }}" required="true" />
                </div>
                ...
                <!-- Module -->
                <div class="mb-6">
                    <x-input-floating type="text" name="module" label="Module" value="{{ old('module', $permission->module) }}" />
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Optional module/feature name (e.g., users)</p>
                </div>

                <!-- Description -->
                <div>
                    <x-textarea-floating name="description" label="Description" value="{{ old('description', $permission->description) }}" />
                </div>
                ...
                <div>
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" id="is_active" value="1"
                            {{ old('is_active', $permission->is_active) ? 'checked' : '' }} class="sr-only peer">
