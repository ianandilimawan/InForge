    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">Add a new user role and define authorization permissions</p>
            </div>
        ...
        <!-- Form Card -->
        <x-card title="Role Information & Access Matrix" subtitle="Specify role identity and configure capability privileges across all system modules.">
            <form x-data="ajaxForm" @submit.prevent="submit" action="{{ route('admin.roles.store') }}" method="POST" class="space-y-6">
                @csrf

                <!-- Basic Information -->
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div class="sm:col-span-2">
                        <x-input type="text" name="name" label="Role Name" value="{{ old('name') }}" :required="true" placeholder="e.g. Content Manager" hint="Unique identifier name for this role." />
                    </div>
                    <div class="sm:col-span-2">
                        <x-textarea name="description" label="Description" value="{{ old('description') }}" placeholder="Brief summary of duties and permissions granted to this role..." hint="Optional description describing the scope of this role." />
                    </div>
                    <div class="sm:col-span-2">
                        <x-toggle name="is_active" label="Role Status" description="When enabled, this role can be assigned to platform users." :checked="old('is_active', true)" variant="card" />
                    </div>
                </div>
