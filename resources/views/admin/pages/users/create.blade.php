    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">Create a new user account with role assignments and granular permissions</p>
            </div>
        ...
        <!-- Form Card -->
        <x-card title="User Details & Credentials" subtitle="Enter personal profile information, authentication credentials, and access privileges.">
            <form x-data="ajaxForm" @submit.prevent="submit" action="{{ route('admin.users.store') }}" method="POST" class="space-y-6">
                @csrf

                <!-- Basic Info -->
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <x-input type="text" name="name" label="Full Name" value="{{ old('name') }}" :required="true" placeholder="e.g. John Doe" hint="The user's full name as displayed across the platform." />
                    <x-input type="email" name="email" label="Email Address" value="{{ old('email') }}" :required="true" placeholder="john.doe@example.com" hint="Active email address used for login and notifications." />
                </div>

                <!-- Password Info -->
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <x-password name="password" label="Password" :strength="true" :required="true" placeholder="••••••••" hint="Must be at least 8 characters with mixed characters." />
                    <x-password name="password_confirmation" label="Confirm Password" :required="true" placeholder="••••••••" hint="Type the exact same password again for confirmation." />
                </div>
                ...
                <div class="pt-6 border-t border-zinc-100 dark:border-zinc-800 space-y-3">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">Assign additional capability permissions beyond the default role access.</p>
                        </div>
                                ...
                                <div class="pb-4 border-b border-zinc-200/60 dark:border-zinc-800 last:border-b-0 last:pb-0">
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                                        @foreach ($modulePermissions as $permission)
                                            <label class="flex items-center p-2 rounded-xl hover:bg-white dark:hover:bg-zinc-800 transition-colors cursor-pointer border border-transparent hover:border-zinc-200/80 dark:hover:border-zinc-700/80 permission-checkbox"
                                                data-module="{{ $module ?? 'other' }}">
