...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700">
            <form x-data="ajaxForm" @submit.prevent="submit" action="{{ route('admin.users.store') }}" method="POST" class="lg:p-8 px-4 py-4 space-y-6">
                @csrf
                ...
                <!-- Email -->
                <x-input-floating type="email" name="email" label="Email" value="{{ old('email') }}" required="true" />
                @error('email')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
                ...
                <div>
                    <div class="relative">
                        ...
                        <label for="password"
                            class="absolute text-sm text-zinc-500 dark:text-zinc-400 duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-white dark:bg-gray-800 px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 start-2 cursor-text">Password</label>
